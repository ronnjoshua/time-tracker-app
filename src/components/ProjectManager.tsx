    ...
    >
      <div className="space-y-4">
        <div className="space-y-2.5">
          ...
          <textarea
            placeholder="Notes (deadlines, scope, contact info...)"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            rows={2}
            className="input-premium w-full px-3 py-2 rounded-lg text-sm text-[var(--foreground)] placeholder-[var(--muted)] resize-none"
          />
            ...
            {PROJECT_COLORS.map((c) => (
              <button
                ...
                className={`w-5 h-5 rounded-full cursor-pointer transition ${
                  color === c ? "ring-2 ring-offset-2 ring-[var(--accent)] ring-offset-[var(--card)]" : ""
                }`}
        ...
        {projects.length > 0 && (
          <div className="border-t border-[var(--card-border)] pt-4 space-y-2">
            {projects.map((project) => (
              <div
                key={project.id}
                className="flex items-center justify-between p-2 rounded-lg hover:bg-[var(--card-border)] transition"
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {project.client_name && (
                        <span className="text-xs text-[var(--muted)] shrink-0">({project.client_name})</span>
                      )}
                    </div>
                    ...
                    {project.notes && (
                      <div className="text-[10px] text-[var(--muted)] mt-0.5 truncate">{project.notes}</div>
                    )}
                  </div>
                </div>
                <div className="flex gap-1 shrink-0 ml-2">
                  <button onClick={() => startEdit(project)} className="p-1 text-[var(--muted)] hover:text-[var(--accent)] transition cursor-pointer">
                  ...
                  <button onClick={() => onDeleteProject(project.id)} className="p-1 text-[var(--muted)] hover:text-[var(--danger)] transition cursor-pointer">
